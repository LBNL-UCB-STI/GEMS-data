...
fit.mfd.unified <- function(input.density, input.flow,  mfd.shape = 'rural'){
  ...
  #            '
  #            'speed-density': for speed density fit
...
# for now fit a parabola
  ...
  if(mfd.shape == 'speed-density'){
    # for now fit speed density
    
    res = lm(data = dat, log(speed) ~ density)
    info.tmp = summary(res)
    R2.fit = info.tmp$r.squared 
    ...
    # so freeflow speed uf = exp(a); optimal density k0 = -1/b; max_capacity = uf*exp(-1)*k0
    freeflow_speed = exp(a) 
    critical_density = -1/b
    return(data.frame(freeflow_speed = freeflow_speed, 
                      critical_density = critical_density,
                      max_capacity = freeflow_speed*exp(-1)*critical_density,
                      R2.fit.speed_density = R2.fit))
    
    
  }
  
  
  if(mfd.shape == 'polynomial'){
    # now fit a flow = phi*density(density - gridlock.density)
...
df = bind_rows(df3.curve,df3.linear) %>%
  bind_rows(df3.urban)
...
mfd.proc.extractv1_exclude3 <- function( filename, spatial_unit = 'blob', upperbound_pct = 0.4, zipTF = F, zipfolder, fnm.inzip){
  # extract v1 data in upper 20percentile of each density bin
  ...
  if(nrow(dat)>0){
    # do not process microtype 3
    if(dat$microtype[1]==3)return(NULL)
    ...
    return(cdat)
  }else{return(NULL)} # if the file contains data
  
}

proc.extractv1_exclude3 <- function( filename, selecttracts,  zipTF = F, zipfolder, fnm.inzip){
  # extract v1 data and exclude microtype 3 for tracts in the partitions "selecttracts"
  ...
  if(nrow(dat)>0){
    ...
    master.dat = dat
      ...
      dat1 = master.dat %>% select(tract_geoid:datetime,contains("V1"), cnt_tmcs )%>% 
        mutate(version = 'V1') %>%
        rename(agg_flow_plph = V1_aggflow_plph,
               agg_density_plpm = V1_aggdensity_plpm,
               aggspeed_mph  = V1_aggspeed_mph )%>%
      ...
      # do not return tracts not in partition
      if(!(as.numeric(master.dat$tract_geoid)[1] %in% selecttracts)){return(NULL)}
      
      
      cdat = master.dat  %>%
        select(tract_geoid, microtype, datetime, agg_flow_plph, agg_density_plpm, aggspeed_mph, cnt_tmcs)
...
proc.extractv1 <- function( filename,  zipTF = F, zipfolder, fnm.inzip){
